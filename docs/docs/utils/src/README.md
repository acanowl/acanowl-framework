[**can-framework**](../../README.md)

***

[can-framework](../../modules.md) / utils/src

# utils/src

## Type Aliases

- [Customizer](type-aliases/Customizer.md)
- [UniversalTimeType](type-aliases/UniversalTimeType.md)

## Functions

- [errorCatch](functions/errorCatch.md)

## Date

- [dateFormat](functions/dateFormat.md)
- [formatDate](functions/formatDate.md)
- [formatDateSeparator](functions/formatDateSeparator.md)
- [parseDateStringWithTime](functions/parseDateStringWithTime.md)

## Object

- [deepClone](functions/deepClone.md)
- [deepMerge](functions/deepMerge.md)

## String

- [getDecimalLength](functions/getDecimalLength.md)

## Valid

- [isArray](functions/isArray.md)
- [isBoolean](functions/isBoolean.md)
- [isDate](functions/isDate.md)
- [isDefined](functions/isDefined.md)
- [isFunction](functions/isFunction.md)
- [isNonZeroNumber](functions/isNonZeroNumber.md)
- [isNull](functions/isNull.md)
- [isNumber](functions/isNumber.md)
- [isObject](functions/isObject.md)
- [isPlainObject](functions/isPlainObject.md)
- [isRegExp](functions/isRegExp.md)
- [isString](functions/isString.md)
- [isValidDate](functions/isValidDate.md)
- [isValidNumber](functions/isValidNumber.md)
