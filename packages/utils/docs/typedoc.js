...
import json2md, { converters } from 'json2md'
...
async function resolveConfig(jsonDir) {
  ...
  // 读取文档数据结构的 json 文件
  const buffer = await readFileSync(jsonDir, 'utf8')
  ...
  await writeFileSync(join(__dirname, 'apidocConfig.json'), JSON.stringify(result), 'utf8')
...
function getClassPath(className) {
  return join(`/${outputDirName}/classes`, `${className}`).replace(/\\/g, '/')
}

function getInterfacePath(interfaceName) {
  return join(`/${outputDirName}/interfaces`, `${interfaceName}`).replace(/\\/g, '/')
}

function getTypePath(typeName) {
  return join(`/${outputDirName}/types`, `${typeName}`).replace(/\\/g, '/')
}

function getFunctionPath(functionName) {
  return join(`/${outputDirName}/functions`, `${functionName}`).replace(/\\/g, '/')
}

function getEnumPath(enumName) {
  return join(`/${outputDirName}/enums`, `${enumName}`).replace(/\\/g, '/')
}
...
async function main() {
  ...
  const handleGenerate = async (project) => {
    if (project) {
      ...
      await writeFileSync(join(outputDir, 'catalogue.md'), catalogueContent, 'utf8')
